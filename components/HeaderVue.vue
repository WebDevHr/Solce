    ...
    <header class="absolute inset-x-0 top-0 z-50">
        <div :class="['fixed w-full transition-all duration-300', hasScrolled ? 'bg-white shadow-md' : 'bg-transparent']">
            <nav class="mx-auto flex max-w-7xl items-center justify-between p-4 lg:px-8">
                <div class="flex rounded-full bg-orange-600">
                    ...
                    <template v-for="item in navItems" :key="item.name">
                        <Popover v-if="item.subItems" class="relative">
                            <template v-slot="{ open }">
                                <PopoverButton
                                    class="flex items-center gap-x-1 text-sm font-semibold leading-6 text-gray-900 hover:text-orange-600 pr-3 pl-5 py-1 rounded-xl border-2 border-transparent hover:border-orange-600 transition-all duration-300">
                                    {{ item.name }}
                                    <ChevronDownIcon
                                ...
                                <PopoverPanel
                                    class="absolute z-10 mt-[22px] border-t-8 border-orange-600 bg-white shadow-lg rounded-xl w-52 overflow-hidden focus:border-none">
                        ...
                        <nuxt-link v-else :to="item.href"
                            class="text-sm font-semibold leading-6 text-gray-900 hover:text-orange-600 px-5 py-1 border-2 rounded-xl border-transparent hover:border-orange-600 transition-all duration-300">
                            {{ item.name }}
                        </nuxt-link>
                    </template>
                </PopoverGroup>
                <div class="hidden lg:flex lg:justify-end">
                    <nuxt-link to="/giris"
                        class="text-sm font-semibold leading-6 text-orange-600 px-5 py-1 border-2 border-orange-600 rounded-xl hover:shadow-md transition-all duration-300 hover:bg-orange-600 hover:text-white flex justify-center items-center gap-x-2">
                        Giriş Yap
